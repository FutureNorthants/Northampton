...
- (IBAction)responseByEMail:(id)sender{
    ...
    if([defaults objectForKey:@"EmailAddress"]&&![[defaults objectForKey:@"EmailAddress"]isEqualToString:@""]){
        ServerResponseViewController *vcServer = [[ServerResponseViewController alloc] initWithNibName:@"ServerResponseViewController" bundle:nil fromContact:fromContact];
        [self.navigationController pushViewController:vcServer animated:YES];
        [vcServer release];
    }else{
        MyDetailsEmailViewController *vcEmail = [[MyDetailsEmailViewController alloc] initWithNibName:@"MyDetailsEmailViewController" bundle:nil fromReport:true];
...
- (IBAction)responseByPhone:(id)sender{
    ...
    if([defaults objectForKey:@"PhoneNumber"]&&![[defaults objectForKey:@"PhoneNumber"]isEqualToString:@""]){
        ...
        MyDetailsPhoneViewController *vcEmail = [[MyDetailsPhoneViewController alloc] initWithNibName:@"MyDetailsPhoneViewController" bundle:nil fromReport:true];
        UIBarButtonItem *backButton = [[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStylePlain target:nil action:nil];
        self.navigationItem.backBarButtonItem = backButton;
        [backButton release];
        [self.navigationController pushViewController:vcEmail animated:YES];
